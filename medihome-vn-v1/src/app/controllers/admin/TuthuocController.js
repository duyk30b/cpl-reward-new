...
class CollectionController {
    ...
    renderList(req, res, next) {
        res.render('admin/tu-thuoc/tuthuoc-list');
    }
    //--[RENDER]--       /list                            -- Render List --
    renderTrash(req, res, next) {
        res.render('admin/tu-thuoc/tuthuoc-trash');
    }
    //--[GET]--       /list                               -- Get List --
    list(req, res, next) {
        Collection.find({ removedAt: 0 })
            .then(results => {
                console.log('🚀 ~ file: TuthuocController.js:16 ~ CollectionController ~ list ~ results:', results)
