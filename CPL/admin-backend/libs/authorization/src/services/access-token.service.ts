...
export class AccessTokenService {
  ...
  create(accessTokenPayload: IAccessTokenPayload) {
    ...
    const exp = parseInt(this.configService.get('access_jwt_exp'))
  ...
  verify(accessToken: string) {
    try {
      ...
      if (e.name == 'TokenExpiredError') {
        throw new BusinessException(TokenError.EXPIRED, HttpStatus.FORBIDDEN)
      } else {
        throw new BusinessException(TokenError.INVALID, HttpStatus.FORBIDDEN)
      }
    }
  }
}
