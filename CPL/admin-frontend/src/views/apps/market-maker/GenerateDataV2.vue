...
export default defineComponent({
  ...
  methods: {
    ...
    async initPairSelected() {
      const systemTargetData = await MarketMakerService.getSystemTarget({})

      let pairConfig: any[] = []
      if (systemTargetData?.status !== HttpStatus.OK) {
        pairConfig =
          CONFIG.MARKET_MAKER_PAIR_LIST?.length > 0
            ? CONFIG.MARKET_MAKER_PAIR_LIST
            : []
      } else {
        pairConfig = Object.keys(systemTargetData.data.data)
      }
