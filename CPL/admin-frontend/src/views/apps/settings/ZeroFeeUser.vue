                    ...
                    <template v-for="(cell, i) in tableHeader" :key="i">
                      ...
                      >
                        ...
                        <slot
                          :name="`cell-${cell.key}`"
                          :row="item"
                          v-else-if="cell.key === 'pairs'"
                        >
                          <span class="text-uppercase">
                            {{ shortStrTradingPair(item.pairs) }}
                          </span>
                        </slot>
...
import ZeroFeeUserModal from './modal/ZeroFeeUserModal.vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData = () => {
      ...
      loading.value = true
      ExchangeHistoryService.getZeroFeeUser(p)
        .then((res) => {
          if (res.data) {
    ...
    onMounted(() => {
      setPageFliud()
      setCurrentPageBreadcrumbs('menu.zeroFeeUser', ['menu.zeroFeeUser'])
      getData()
      getListPair()
    })
    ...
    const onDelete = (item) => {
      ElMessageBox.confirm('Are you sure to delete?').then(() => {
        loading.value = true
        ExchangeHistoryService.updateZeroFeeUser({
          user_id: item.user_id,
          email: item.email,
          pairs: [],
        }).then((res) => {
    ...
    const listPairs = ref<any[]>([])

    const getListPair = async () => {
      const res = await ExchangeHistoryService.getPairList()

      if (res && res.data && res.data.data) {
        listPairs.value = res.data.data
      }
    }

    const isAllPairs = (pairs: string) => {
      let checkAll = true

      const userPairs = pairs.split(',').length
        ? pairs.split(',').map((p) => {
            const pair = p.split('/')
      ...
      listPairs.value.forEach((p) => {
        if (!currencyList.includes(p.currency)) return

        const isChecked = userPairs.find(
    ...
    const shortStrTradingPair = (pairs: string) => {
      const n = 25
      const str =
        pairs && pairs.length > n ? pairs.slice(0, n - 1) + '...' : pairs

      return isAllPairs(pairs) ? 'All pairs' : str
    }
