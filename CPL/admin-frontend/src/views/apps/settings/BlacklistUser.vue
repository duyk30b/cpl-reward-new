...
import BlackListUserModal from './modal/BlackListUserModal.vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const getData = () => {
      ...
      loading.value = true
      ExchangeHistoryService.getBlacklistUser(p)
        .then((res) => {
          if (res.data) {
    ...
    const onDelete = (item) => {
      ElMessageBox.confirm('Are you sure to delete?').then(() => {
        loading.value = true
        ExchangeHistoryService.deleteBlacklistUser(item.user_id).then((res) => {
