  ...
  data() {
    return {
      ...
      tableConfig: {
        ...
        dataSource: (params) =>
          HighLowService.getSuspensionUsers({
            ...params,
            ...this.query,
          }),
        columns: [
          ...
          },
          {
            key: 'action',
            title: '',
            class: 'text-center td-w-150px min-w-150px',
          },
        ],
        searchColumns: [],
        configPage: true,
        ...
        activeExport: true,
        onSearch: true,
        tableName: 'suspension-common-table',
        createExport: async (params) => {
          ...
          let data = await HighLowService.exportSuspensionCommon(query)
    ...
    removeSuspension: async function (id: number) {
      if (id) {
        const { isConfirmed } = await Swal.fire({
          text: 'Do you want to unlimit this limit?',
          icon: 'question',
