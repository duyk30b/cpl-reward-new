<template>
  <div
    class="modal fade"
    id="kt_modal_user_grant_payout_modal"
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      ...
      <div class="modal-content">
        ...
        <div class="modal-header" id="kt_modal_add_unlimited_user_header">
          <!--begin::Modal title-->
          <h2 class="fw-bolder">{{ $t('highLow.grantPayout') }}</h2>
          <!--end::Modal title-->

          <!--begin::Close-->
          <div
            id="kt_modal_user_grant_payout_close"
        ...
        <div class="m-6">
          ...
          <v-select
            ...
            @change="changeSelect"
            :remote="true"
            :sourceFunction="getRemoteOptions"
          >
          </v-select>
        </div>
        <div class="m-6">
...
export default defineComponent({
  name: 'add-user-grant-payout-modal',
  ...
  methods: {
    getErrorMessage: function (messageKey: string): string {
      switch (messageKey) {
        case 'BO_USER_GRANT_PAYOUT.EXISTS':
          return 'This user has been granted payout. Please select another. '
        default:
          return 'Some things was wrong. Please contact administrator to resolve.'
      }
    },
    ...
    async handleGrantPayoutUser() {
      ...
      this.isLoading = true
      const res = await HighLowService.addUserGrantPayout({
      ...
      if (res.data['success']) {
        ...
        this.dataUser.payoutBonus = ''
        this.$emit('refreshTable')
      } else {
        const { message } = res.data
        this.$toastr.error(this.getErrorMessage(message))
      }
    },
    async getRemoteOptions(text) {
      const response = await UserService.searchByFilter({
        email: text,
      })
