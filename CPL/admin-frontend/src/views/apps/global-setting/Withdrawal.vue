                        ...
                        <slot :name="`cell-${cell.key}`" :row="item">
                          ...
                          >
                            <input
                              ...
                              @change="editWithdrawStatus(item, $event)"
                            />
                          </div>
                          <span v-else>
                            {{ item[cell.key] }}
        ...
        <div class="row">
          ...
          >
            <el-pagination
              ...
              @current-change="getWithdrawSetting"
...
import { TRANSACTION_TYPE_WITHDRAW } from '@/views/currency/variables/currency.const'
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      tableHeader: [
        ...
        {
          name: 'walletGeneral.withdrawEnable',
          key: 'withdraw_enable',
        },
      ],
      listSetting: [],
  ...
  mounted() {
    setPageFliud()
    setCurrentPageBreadcrumbs('menu.globalWithdrawSetting', [
      'menu.globalSetting',
    ])
  ...
  methods: {
    async getWithdrawSetting() {
      ...
      this.listSetting = result
        ...
        .map((item) => {
          const settingWithdraw = item.settings.find(
            (setting) =>
              setting.key ===
              `${item.chain_code.toUpperCase()}_WITHDRAW_ENABLE`,
          )

          return {
            coin: item.chain_code,
            withdraw_enable: settingWithdraw
              ? settingWithdraw?.value === 'true'
              : true,
          }
        })
    ...
    async editWithdrawStatus(item, event) {
      const params = {
        coin: item.coin,
      ...
      const confirmMessage = params.status
        ? 'currencyScreen.enableWithdrawalConfirm'
        : 'currencyScreen.disableWithdrawlConfirm'
      ...
      if (!isConfirmed) {
        item.withdraw_enable = !params.status
        return
      }
