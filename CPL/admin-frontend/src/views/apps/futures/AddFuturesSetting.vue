...
export default defineComponent({
  name: 'add-futures-setting',
  components: { Form, Field, ErrorMessage },
  beforeMount() {
    this.checkPairLocalStorage()
  },
  methods: {
    isDigit(e) {
      return isDigit(e)
    ...
    checkPairLocalStorage() {
      const coin = localStorage.getItem('future_setting_coin')
      const currency = localStorage.getItem('future_setting_currency')

      if (coin == null || coin === '' || currency == null || currency === '') {
        this.removePairLocalStorage()
        this.$router.push({ name: 'futures.setting' })
        return
      }

      this.submitData.coin = coin
      this.submitData.currency = currency
      this.submitData.pairName = `${coin}/${currency}`
    },
    removePairLocalStorage() {
