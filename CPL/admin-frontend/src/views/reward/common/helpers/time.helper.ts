export const timeToText = (
  ...
  const date = new Date(time.getTime() - timezoneOffset * 60000)
  const rules = {
...
export const textToTime = (text: string, pattern: string): Date => {
  ...
  const minutes =
    iMinutes !== -1 ? Number(text.slice(iMinutes, iMinutes + 2)) : 0
  const seconds =
    iSeconds !== -1 ? Number(text.slice(iSeconds, iSeconds + 2)) : 0
