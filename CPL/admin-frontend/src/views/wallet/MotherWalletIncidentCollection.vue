        ...
        <div class="row mb-4">
          ...
          <div class="col-3 mr-8">
            ...
            <span style="color: #f56c6c" v-if="isNotValid()">
              This field must be greater than {{ minValue }}
            </span>
          </div>
        </div>
        <div class="row mb-4">
...
export default defineComponent({
  setup() {
    ...
    const getData = (params?) => {
      ...
      // }
      if (isNotValid() || isNotValidCoin()) return
      ...
      WalletSettingService.getIncident(p).then((res) => {
        if (res.data) {
          ...
          items.value = responseData.map((item) => {
            ...
            item.balanceF = item.balance.minus(item.pending_balance) || 0
            return item
          })
          totalB.value =
            res.data.meta_data &&
    ...
    const blurInput = () => {
      collectionThreshold.value = new BigNumber(collectionThreshold.value)
        .toFixed(20)
        .toString()
        .replace(/\.?0+$/, '')
    }
