  ...
  setup() {
      ...
      WalletBceService.getExternalWithdrawTransactions(p).then((res) => {
        if (res.data) {
          ...
          items.value = responseData.map((item) => {
            ...
            item.withdrawal_type = item.withdrawal_type
              ? item.withdrawal_type === 'handle_auto_withdraw_manually'
                ? t('walletGeneral.autoWithdrawFail')
    ...
    onMounted(() => {
      ...
      setCurrentPageBreadcrumbs(
        'menu.walletSub.withdrawalControl.approvalPending',
