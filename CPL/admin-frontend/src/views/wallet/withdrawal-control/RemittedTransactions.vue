                  ...
                  <tr class="odd">
                    ...
                    <td class="d-flex justify-content-between gap-2">
                      <button
                        ...
                        @click="openDetail(item.transaction_id)"
                      >
                        <span>
...
export default defineComponent({
  ...
  setup() {
    const tableHeader = ref([
      ...
      {
        name: 'walletGeneral.executionDate',
        key: 'sent_at',
        sortable: true,
        minWidth: 150,
      },
      {
        name: 'walletGeneral.processType',
        key: 'process_type',
        sortable: true,
        minWidth: 180,
      },
      {
        name: 'walletGeneral.processSourceType',
        key: 'withdrawal_type',
        sortable: true,
        minWidth: 180,
      },
      {
        name: 'walletGeneral.withdrawalApprover',
        ...
        if (res.data) {
          ...
          items.value = responseData.map((item) => {
            item.sent_at = item.sent_at
              ? moment.unix(+item.sent_at / 1000).format('YYYY-MM-DD HH:mm')
              : ''
            ...
            const withdrawalType = item.withdrawal_type

            item.withdrawal_type = withdrawalType
              ? withdrawalType === 'auto'
                ? t('walletGeneral.autoWithdraw')
                : t('walletGeneral.manualWithdraw')
              : ''

            item.process_type = item.process_type
              ? item.process_type === 2
