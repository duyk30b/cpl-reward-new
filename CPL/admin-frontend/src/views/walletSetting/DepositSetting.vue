...
export default defineComponent({
  ...
  setup() {
    ...
    const coinEdit = ref()
    const openEdit = (coin) => {
      titleModal.value = t('menu.walletSettingSub.depositSetting')
    ...
    const formatData = (res: SettingData[]) => {
      ...
      res.forEach((item) => {
        ...
        itemSettingSorted.forEach((s) => {
          let key = s.key.slice(item.chain_code.length + 1, s.key.length)
          row[key] = s.value
          row.show = s.show
        })
        result = [...result, row]
    ...
    onMounted(() => {
      setPageFliud()
      setCurrentPageBreadcrumbs('menu.walletSettingSub.depositSetting', [
        'menu.walletSettingSub.depositSetting',
      ])
