<template>
  <div class="card">
    <div class="card-header border-0 pt-6 mb-3">
      <div class="card-title">{{ $t('dividendScreen.dividendEdit') }}</div>
    </div>
    ...
    <div class="card-body">
      ...
      >
          ...
          <div class="col-6">
            <div class="row mt-5 align-items-center">
              ...
              <div class="col-8">
                ...
                <ErrorMessage name="dividend_name" class="text-danger" v-else />
              </div>
            </div>
            ...
            <div class="row mt-5 align-items-center">
              ...
              <div class="col-8">
                ...
                <ErrorMessage name="distributed_currency" class="text-danger" />
              </div>
            </div>
            ...
            <div class="row mt-5 align-items-center">
              <div class="col-8 offset-4 d-flex">
                ...
                <label
                  ...
                  >{{ $t('dividendScreen.unlimitedJoinedUserMaximum') }}</label
                >
              </div>
            </div>
          ...
          <div class="col-6">
            ...
            <div class="row mt-5 align-items-center">
              <div class="col-8 offset-4 d-flex">
                ...
                <label class="fw-bolder ms-2" for="unlimited-end-date-checkbox">
              ...
              <div class="col-8 input-unit">
                ...
                <span
                  ...
                  v-if="
                    ...
                    dividend.dividend_type === DISTRIBUTE_TYPE.INDICATED_BALANCE
                  "
                  >USD</span
                >
                <span
        ...
        <div class="mt-5 d-flex justify-content-center">
          <button class="btn btn-primary me-5" @click.prevent="goBack">
            {{ $t('back') }}
          </button>
          <button
            type="button"
...
export default defineComponent({
  ...
  methods: {
    async getDividend() {
      ...
      if (success) {
        ...
        this.dividend.id = dividend.id
        this.dividend.dividend_name = dividend.name
        this.dividend.unlimited_end_date = dividend.unlimited_end_date
        this.dividend.target_currency = dividend.target_currency
        this.dividend.distributed_currency = dividend.distributed_currency
        this.dividend.dividend_span = dividend.dividend_span
        this.dividend.dividend_date.date = dividend.dividend_date_day
        this.dividend.dividend_date.month = dividend.dividend_date_month
        this.dividend.dividend_calculated_mode =
          dividend.dividend_calculated_mode

        const hour = dividend.dividend_date_hour
        const minute = dividend.dividend_date_minute
        this.dividend.executing_time = new Date(
          new Date().setUTCHours(hour, minute, 0),
        )

        this.dividend.dividend_type = dividend.distribute_type
        this.dividend.start_date = moment(dividend.start_date).format(
          'YYYY-MM-DD HH:mm',
        )
        this.dividend.end_date = moment(dividend.end_date).format(
          'YYYY-MM-DD HH:mm',
        )
        this.unlimitJoinedUserMaximum = dividend.is_limited ? false : true
        this.unlimitDividendMaximum = dividend.dividend_limited ? false : true
        this.dividend.joined_user_maximum = dividend.maximum_user_numbers
        this.dividend.indicated_amount = this.removeCommaCurrency(
          formatCurrencyAmount(dividend.distribute_amount, null, null, null),
        )
        this.dividend.dividend_maximum = this.removeCommaCurrency(
          formatCurrencyAmount(
            dividend.maximum_dividend_amount,
            null,
            null,
            null,
          ),
        )
        this.dividend.dividend_rate = this.removeCommaCurrency(
          formatCurrencyAmount(
            dividend.dividend_calculated_mode ===
              DIVIDEND_CALCULATED_MODE.NON_CONDITION
              ? dividend.dividend_rate
              : numeral(dividend.dividend_rate * 100).format('0.00000000'),
            null,
            null,
            null,
          ),
        ...
        // after asign for dividendInit
        this.$nextTick(() => {
          this.dividendFromServer = { ...data.data }
        })
      }
    ...
    async onSubmit() {
      ...
      const { success, data } = await DividendService.editDividend(
        dividendParams,
      )
      if (success && data.data.success) {
        this.$router.push({ name: 'dividend.listScreen' })
    ...
    async checkDividendNameExist(dividendName) {
      ...
      if (dividendName) {
        const { data } = await DividendService.getDividendWithoutId(
          this.dividend.id,
        )
  ...
  watch: {
    ...
    'dividend.dividend_type': function () {
      if (this.dividendFromServer) {
        if (this.dividend.dividend_type !== DISTRIBUTE_TYPE.INDICATED_BALANCE) {
          this.dividend.indicated_amount = ''
        } else {
          this.dividend.indicated_amount = formatCurrencyAmount(
            !(this.dividendFromServer as any).distribute_amount,
    ...
    'dividend.unlimited_end_date': function () {
      if (this.dividendFromServer) {
        if (this.dividend.unlimited_end_date) {
          ...
          this.dividend.end_date =
            this.dividend.end_date ||
            moment(this.dividend.start_date)
              .add(7, 'days')
