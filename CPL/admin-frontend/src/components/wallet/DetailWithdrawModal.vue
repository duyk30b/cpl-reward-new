<template>
  <div
    class="modal fade"
    id="kt_detail_withdraw_modal"
    ref="detailWithdrawModalRef"
    tabindex="-1"
    aria-hidden="true"
  >
              ...
              >
                ...
                >
                  <div class="accordion-body">
                    ...
                    <div v-if="log?.overwrite_action">
                      <div class="row my-4">
                        <div class="col-4 d-flex align-items-center fw-bold">
                          {{ $t('walletGeneral.overwriteAction') }}:
                        </div>
                        <div class="col-6">
                          <span
                            class="badge"
                            :class="classAction[log.overwrite_action]"
                          >
                            {{ log.overwrite_action }}
                          </span>
                        </div>
                      </div>
                      <div class="row mb-4">
                        <div class="col-4 d-flex align-items-center fw-bold">
                          {{ $t('walletGeneral.overwriteBy') }}:
                        </div>
                        <div class="col-6">
                          {{ log?.overwrite_admin?.email }}
                        </div>
                      </div>
                      <div class="row mb-4">
                        <div class="col-4 d-flex align-items-center fw-bold">
                          {{ $t('walletGeneral.overwriteAt') }}:
                        </div>
                        <div class="col-6">
                          {{ log?.overwrite_created_at }}
                        </div>
                      </div>
                      <div class="row mb-4">
                        <div class="col-4 d-flex align-items-center fw-bold">
                          {{ $t('walletGeneral.overwriteLog') }}:
                        </div>
                        <div class="col-6">
                          {{ log?.overwrite_message }}
                        </div>
                      </div>
                    </div>
                  </div>
...
export default defineComponent({
  ...
  methods: {
    async getTransactionDetail() {
      this.loading = true
      try {
        const res = await WalletSettingService.getDetailWithdrawTransaction(
          this.transactionId,
        )
        ...
        this.logs = res.data.data?.logs
          ? res.data.data?.logs.map((item) => ({
              ...item,
              created_at: convertDateFormat(item.created_at, 'x'),
              overwrite_created_at: convertDateFormat(
                item.overwrite_created_at,
                'x',
              ),
            }))
          : []
    ...
    closeModal() {
      hideModal(this.$refs.detailWithdrawModalRef as HTMLElement)
      this.$emit('close')
    },
    formatNumber(number) {
      if (!number) {
