    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
      <div class="modal-content">
        <div class="modal-header">
          <h2 class="fw-bolder">{{ isEdit ? $t('update') : $t('create') }}</h2>
          <div
                ...
                <span
                  ...
                  >{{
                    itemEdit.email && itemEdit.email.length >= 1
                      ? $t('walletGeneral.emailIsInvalid')
                      : $t('walletGeneral.emailIsRequired')
                  }}</span
                >
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async querySearch(queryString, cb) {
      const results = await WalletSettingService.queryBlacklistUser({
        keyword: this.itemEdit.email,
      })
      cb(
        results &&
          results.data &&
    ...
    onClick() {
      ...
      if (this.isEdit) {
        ...
        ElMessageBox.confirm(this.$t('walletGeneral.areYouSureToUpdate')).then(
          () => {
            ...
            this.loading = true
            WalletSettingService.updateBlacklistUser(param)
              .then((res) => {
      ...
      ElMessageBox.confirm(this.$t('walletGeneral.areYouSureToAdd')).then(
        () => {
          ...
          WalletSettingService.addBlacklistUser(param).then((res) => {
            if (res && res.status === 200) {
              this.$emit('updateData')
              this.$toastr.success(this.$t('success'))
              hideModal(this.$refs.blackListModalRef)
