<template>
  <base-modal
    title="Amazon Rekognition"
    :show="show"
    @close="close"
    dialogClass="modal-xl"
  >
    <template v-slot:body>
      <ul
        class="nav nav-stretch nav-line-tabs fw-bold border-bottom"
        role="tablist"
      >
        <li class="nav-item" v-for="tab in availableTabs" :key="tab">
          <a
            class="nav-link text-uppercase"
            :class="{ active: tab == currentTab }"
            data-bs-toggle="tab"
            href="javascript:void(0)"
            @click="chooseTab(tab)"
            role="tab"
          >
            {{ $t(`imageProcessDetailTab.${tab}`) }}
          </a>
        </li>
      </ul>
      <div class="tab-content py-8 border-start border-end border-bottom px-6">
        <div
          class="tab-pane"
          :class="{
            active: currentTab == ImageProcessDetailTab.COMPARE_STATUS,
          }"
        >
          <amazon-rekognition-compare-status
            :userKyc="userKyc"
            :rekognitionInfoHistory="rekognitionInfoHistory"
            :rekognitionInfoHistoryDetail="rekognitionInfoHistoryDetail"
          ></amazon-rekognition-compare-status>
        </div>
        <div
          class="tab-pane"
          :class="{
            active: currentTab == ImageProcessDetailTab.DUPLICATE_STATUS,
          }"
        >
          <amazon-rekognition-duplicate-status
            :userKyc="userKyc"
            :rekognitionInfoHistory="rekognitionInfoHistory"
            :rekognitionInfoHistoryDetail="rekognitionInfoHistoryDetail"
          ></amazon-rekognition-duplicate-status>
        </div>
      </div>
    </template>
    <template v-slot:footer>
      <button class="btn btn-secondary" type="button" @click="close">
        {{ $t('close') }}
      </button>
    </template>
  </base-modal>
</template>

<script lang="ts">
import { defineComponent, PropType } from 'vue'
import {
  ImageProcessResult,
  KycImageProvider,
  RekognitionInfoHistory,
  RekognitionInfoHistoryDetail,
} from '@/models/user/UserKycScanData'
import { UserKyc, UserKycHistory } from '@/models/user/UserKyc'
import BaseModal from '@/components/modals/BaseModal.vue'
import { ImageProcessDetailTab } from '../image-process-detail.const'
import { Optional } from '@/models/common/Optional'
import { UserKycService } from '@/services/UserKycService'
import AmazonRekognitionCompareStatus from './AmazonRekognitionCompareStatus.vue'
import AmazonRekognitionDuplicateStatus from './AmazonRekognitionDuplicateStatus.vue'

export default defineComponent({
  emits: ['close'],
  name: 'amazon-rekognition-detail',
  components: {
    BaseModal,
    AmazonRekognitionDuplicateStatus,
    AmazonRekognitionCompareStatus,
  },
  mounted() {
    this.chooseTab(this.initTab)
  },
  props: {
    result: ImageProcessResult,
    userKyc: Object as PropType<UserKyc | UserKycHistory>,
    show: Boolean,
    initTab: {
      type: String,
      default: ImageProcessDetailTab.COMPARE_STATUS,
    },
  },
  watch: {
    show: function () {
      this.chooseTab(this.initTab)
    },
    initTab: function () {
      this.chooseTab(this.initTab)
    },
    userKycHistoryId: {
      handler() {
        this.getData()
      },
      immediate: true,
    },
  },
  data() {
    return {
      ImageProcessDetailTab,
      KycImageProvider,
      currentTab: ImageProcessDetailTab.COMPARE_STATUS as unknown as string,
      rekognitionInfoHistory: null as Optional<RekognitionInfoHistory>,
      rekognitionInfoHistoryDetail:
        null as Optional<RekognitionInfoHistoryDetail>,
      availableTabs: [
        ImageProcessDetailTab.COMPARE_STATUS,
        ImageProcessDetailTab.DUPLICATE_STATUS,
      ],
    }
  },
  computed: {
    userKycHistoryId() {
      return this.userKyc?.userKycHistoryId
    },
  },
  methods: {
    chooseTab(tab: string) {
      this.currentTab = tab
    },
    close() {
      this.$emit('close')
    },
    async getRekognitionInfoHistory() {
      if (!this.userKycHistoryId) {
        this.rekognitionInfoHistory = null
      } else {
        this.rekognitionInfoHistory =
          await UserKycService.findRekognitionInfoHistoryByKycHistoryId(
            this.userKycHistoryId,
          )
      }
    },
    async getRekognitionInfoHistoryDetail() {
      if (!this.rekognitionInfoHistory) {
        this.rekognitionInfoHistoryDetail = null
      } else {
        this.rekognitionInfoHistoryDetail =
          await UserKycService.findRekognitionInfoHistoryDetail(
            this.rekognitionInfoHistory.id,
          )
      }
    },
    async getData() {
      await this.getRekognitionInfoHistory()
      await this.getRekognitionInfoHistoryDetail()
    },
  },
})
</script>
