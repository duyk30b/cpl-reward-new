...
export const greyColor = function (min, max) {
  min = min || 1
  max = max || 9
...
export const captchaText = function (options) {
 ...
 */
export const mathExpr = function (min, max, operator) {
  min = min || 1
  max = max || 9
  operator = operator || '+'
...
export const color = function (bgColor = undefined) {
  // Random 24 colors
  // or based on step
  const hue = randomInt(0, 24) / 24

  const saturation = randomInt(60, 80) / 100
  const bgLightness = bgColor !== undefined ? getLightness(bgColor) : 1.0
  let minLightness
  let maxLightness
  if (bgLightness >= 0.5) {
